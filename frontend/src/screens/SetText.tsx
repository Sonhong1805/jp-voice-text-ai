...
import { Save, X, Search, Volume2, AlertCircle, CheckCircle2, History, Upload, Download, Music, Users } from 'lucide-react';
...
export default function SetText({ initialText, onSave }: SetTextProps) {
  ...
  const handleExportAudio = async () => {
    ...
    try {
      ...
      alert('Có lỗi xảy ra khi tải âm thanh. Vui lòng thử lại.');
    } finally {
  ...
  const playTTS = () => {
    ...
    if (jaVoice) {
      ...
      alert('Thiết bị chưa có giọng đọc tiếng Nhật. Âm thanh nghe được có thể không chính xác (chỉ dùng để tham khảo).');
    }
  ...
  const selectSavedSentence = (text: string) => {
    ...
    setTextError('');
    if (text.length > 50) {
      setTextWarning(`Câu khá dài (${text.length} ký tự). Bạn nên chia thành 2-3 câu để luyện phát âm dễ hơn.`);
    } else {
      setTextWarning('');
    }
  };
