...
async def score_pronunciation(audio: UploadFile = File(...), expected_text: str = Form("")):
    ...
    # --- Generate TTS Reference Audio for Prosody (DTW) ---
    ref_audio_data = None
    if expected_text.strip():
        try:
            communicate = edge_tts.Communicate(expected_text, voice="ja-JP-NanamiNeural")
            ref_audio_bytes = b""
            async for chunk in communicate.stream():
                if chunk["type"] == "audio":
                    ref_audio_bytes += chunk["data"]
            ref_audio_data = audio_validator.validate_and_decode(ref_audio_bytes)
        except Exception as e:
            print(f"Failed to generate TTS ref audio: {e}")
            pass
...
# Mount assets specifically so they are served correctly
...
async def serve_frontend(full_path: str):
    # Check if the requested file exists in dist (e.g. vite.svg, robots.txt)
